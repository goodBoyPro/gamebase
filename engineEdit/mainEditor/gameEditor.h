...
namespace ens
{
    ...
    public:
        ...
        void loop()
        {
            ...
            while (window.isOpen())
            {
                ...
                // 渲染///////////////////////////////////////////////////////////////////////////
                window.clear();
                const wchar_t *showmode[3] = {L"游戏模式", L"ACTOR模式", L"LAND模式"};
                const wchar_t *temp;
                switch (editMode)
                {
                case GAMEMODE:
                ...
                for (auto obj : allObj)
                {
                    if (editMode == LANDMODE && obj->type == MovableEditObj::elandBlock)
                    {
                        obj->drawHandler(window);
                    }
                    else if (editMode == ACTORMODE && obj->type == MovableEditObj::eactor)
                    {
                        obj->drawHandler(window);
                    }
                }
    ...
    inline void Editor::setCommand()
    {
        editorCommand::edc.command["cr"] = [&]()
        {
            ...
            if (editMode == ACTORMODE)
                obj = createAtLocation(new Actor(std::stoi(editorCommand::edc.input[1]),
                                                 std::stoi(editorCommand::edc.input[2])),
                                       WindowFlag::flag.posInWs);
            else if (editMode == LANDMODE)
                obj = createAtLocation(new LandBlock(std::stoi(editorCommand::edc.input[1]),
                                                     std::stoi(editorCommand::edc.input[2])),
                                       WindowFlag::flag.posInWs);
